...
class Help_Command:
    ...
    async def moderator(self, ctx):
        ...
        em.add_field(name="dmf", value=";dmf <user> / <user id>  |  Send that, then when I say 'please type your message now' then send another message with the text you want me to send that person. Once I have done it, I will send a message saying 'Your message has been sent'. This DM command allows you to send messages that are 2000 characters.", inline=False)
        em.add_field(name="dm", value=";dm <user> / <user id>  [message]  |  This DM command is the shortened version of ';dmf'. This command should be quicker and easier to send messages to someone because you only need to write 1 line of stuff (command and message in the same line) instead of having to write 2 lines of stuff (command in the first line, then the message in the second.", inline=False)
